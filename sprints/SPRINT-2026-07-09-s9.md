...
- [x] **T9 — Direct vault write.** *(PR #9, merged 2026-07-10 — File System Access API vault write; wiz `/song/note` send in live session mode)* "Save session to vault": `postToWiz` path when a :3341 session is live; File System Access API fallback (directory picked once) in static mode. Scope finalized at build time per the note.

### Close
- [x] Update workspace `project-roadmap.md` — mark shipped, remove `[in-sprint]`
- [x] Update `README.md` + `ROADMAP.md` for esplumoir-albrecht
- [x] Update `esplumoir-albrecht/CLAUDE.md` — new localStorage keys, session log gate, URL param precedence, audition, vault write, Modal Colors, diatonic triad fix
- [x] Archive this file → `sprints/SPRINT-2026-07-09-s9.md`
- [x] Git commit, then run `node agents/orchestrator.js sync`
